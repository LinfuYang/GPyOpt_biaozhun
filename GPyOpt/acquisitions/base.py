...
class AcquisitionBase(object):
    ...
    def optimize(self, temp_t=None, duplicate_manager=None):
        """
        Optimizes the acquisition function (uses a flag from the model to use gradients or not).
        """
        return self.optimizer.optimize(temp_t=temp_t, f=self.acquisition_function, duplicate_manager=duplicate_manager)
