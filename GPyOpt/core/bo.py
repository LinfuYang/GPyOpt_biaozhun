...
class BO(object):
    ...
    def run_optimization(self, max_iter=0, max_time=np.inf,  eps=1e-8, context=None, verbosity=False,
        ...
        while (self.max_time > self.cum_time):
            ...
            # UCB 参数控制explore or exploit
            t = self.num_acquisitions + 1
            sigma = 0.4186
            One = 2 * np.log2((t ** 2) * np.pi / (6 * sigma))
            temp_t = np.sqrt(One)

            self.suggested_sample = self._compute_next_evaluations(temp_t)
    ...
    def _compute_next_evaluations(self, temp_t, pending_zipped_X=None, ignored_zipped_X=None):
        """
        ...
        return self.space.zip_inputs(self.evaluator.compute_batch(tempt=temp_t, duplicate_manager=duplicate_manager, context_manager=self.acquisition.optimizer.context_manager))
    ...
    def plot_convergence(self, filename=None, func_name=None):
        """
        ...
        :param filename: name of the file where the plot is saved
        """
        return plot_convergence(self.X, self.Y_best, filename, func_name)
