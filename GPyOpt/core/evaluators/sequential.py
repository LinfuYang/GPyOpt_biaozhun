...
class Sequential(EvaluatorBase):
    ...
    def compute_batch(self, tempt=None, duplicate_manager=None, context_manager=None):
        """
        Selects the new location to evaluate the objective.
        """
        x = self.acquisition.optimize(temp_t=tempt, duplicate_manager=duplicate_manager)
        return x
