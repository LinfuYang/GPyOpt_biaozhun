...
class AcquisitionOptimizer(object):
    ...
    """
        # 加入cluster_k, point_num
    def __init__(self, space, optimizer='lbfgs', cluster_k=None,  point_num=None, **kwargs):
        self.space              = space
    ...
    def optimize(self, temp_t=None, f=None, duplicate_manager=None):
        """
        ...
        self.optimizer = choose_optimizer(self.space, self.optimizer_name, self.context_manager.noncontext_bounds)
...
#
        ...
        # 自己写：
        x_min = self.optimizer.optimize(tempt=self.temp_t, f=self.f, k=self.cluster_k, point_num=self.point_num)
